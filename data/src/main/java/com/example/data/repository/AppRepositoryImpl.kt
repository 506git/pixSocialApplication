...
//                    val databaseRef = firebaseDatabase.getReference("userRoomInfo").child(currentUserUid).child("@make@$currentUserUid@${checkUser.key}")
                    ...
                    val databaseMy = databaseRef.child(currentUserUid).child("@make@$tempCurrentUserId@${tempOtherUserId}")
                    val databaseOther = databaseRef.child(otherUserId).child("@make@$tempCurrentUserId@${tempOtherUserId}")
                    databaseMy.setValue(
                        UserRoomInfo(room_img = checkUser.child("imageUrl").value.toString(),
                    ...
                    databaseOther.setValue(
                        UserRoomInfo(room_img = currentUser?.photoUrl.toString(),
                            room_title = currentUser?.email.toString(),
                            room_name = currentUser?.displayName.toString(),
                            room_id = "@make@$tempCurrentUserId@${tempOtherUserId}"
                        )
                    )
                    firebaseDatabase.getReference("roomInfo").child("@make@$tempCurrentUserId@${tempOtherUserId}").setValue(
                        ""
                    )
                }
                trySend(Result.Success(Unit))
    ...
    override suspend fun sendChat(message: String, roomId: String): Flow<Result<Unit>> = callbackFlow{
        send(Result.Loading())
        val currentUser = auth.currentUser
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    override suspend fun sendImage(message: String, roomId: String): Flow<Result<Unit>> = callbackFlow {
        send(Result.Loading())
        val currentUser = auth.currentUser
        val databaseReference = firebaseDatabase.getReference("roomInfo")
...
//    }
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    fun simpleDateFormat(date : LocalDateTime)= date.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))
